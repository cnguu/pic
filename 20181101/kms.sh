...
install_main() {
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://cdn.jsdelivr.net/gh/cnguu/pic@master/20181101/kms; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms https://cdn.jsdelivr.net/gh/cnguu/pic@master/20181101/kms-debian; then
            echo -e "[${red}Error:${plain}] Failed to download KMS Server script."
            exit 1
        fi
    else
    ...
    cd "${cur_dir}" || exit
    git clone https://git.dev.tencent.com/cnguu/vlmcsd.git > /dev/null 2>&1
